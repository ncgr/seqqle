...
For Rails >= 3.1 applications, we recommend the {Quorum}[https://github.com/ncgr/quorum] gem.
...
* JRuby version 1.6.4 http://jruby.org/ (tested under v1.6.4)
...
NOTE:
If you choose to execute bin/run-blast remotely via SSH, make sure the dependancies listed above
(Emboss, MySQL, NCBI Blast) are exported in the remote machine's .bashrc file. Otherwise
run-blast will return error code 73 (binary not found).
